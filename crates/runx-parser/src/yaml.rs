...
fn top_level_plain_key(trimmed: &str) -> Option<(&str, usize)> {
    ...
    if bytes
        .first()
        .is_some_and(|byte| matches!(*byte, b'?' | LEFT_BRACE_BYTE | b'[' | b'"' | b'\''))
    {
        return None;
    }
...
mod tests {
    ...
    #[test]
    fn parity_subset_rejects_embedded_colon_in_key_starting_with_dash() {
        let result = assert_yaml_parity_subset("fixture", "-key: value:with:colon");
        assert!(result.is_err(), "expected rejection, got {:?}", result);
    }
}
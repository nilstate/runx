...
Authority bounds the run. The `wallet:spend <= spend_limit` scope keeps the
settlement inside the supplied limit and `ledger:append` records the settlement
intent; no broader wallet scope is requested or implied. A `ready` decision means
the plan is well-formed and within bound, not that money may move; the approval
...
rail, spend limit, the decision, the gate state, and the blocker list as proof,
and never a full account number, card PAN, funding credential, or raw secret.
...
`acct_3f9c1a7e`, under a `spend_limit` of 2500.00, on the `ach` rail.

Output: `decision: ready`; the amount fits the bound, so no overage blocker; the

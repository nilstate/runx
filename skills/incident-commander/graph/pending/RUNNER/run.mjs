...
if (inputs.incident_objective === "assign" && !state.named_owner) {
    ...
    `${JSON.stringify({
      incident_turn: {
        ...
        escalation: {
          ...
          reason: "assign requires a named roster owner",
        },
        named_run: null,
        reason: "no named owner exists in the folded incident case",
      },
    })}\n`,
  );
  process.exit(0);
...
const incidentTurn = valid
  ...
  : {
      ...
      escalation: {
        lane: "human:incident-reviewer",
        reason: "invalid pending communications state",
      },
      named_run: null,

...
const proposedTurn = inputs.proposed_turn ?? null;
...
const reviewError =
  !proposedTurn || proposedTurn.case_id !== inputs.case_id || proposedTurn.turn !== base.turn
    ? "review act did not return the current incident turn"
    : null;
if (!inputs.case_id || !inputs.driver_id || !allowedObjectives.has(objective)) {
  ...
  result = refused(base, reviewError);
...
} else if (objective === "resolve" && !memberClosureRef && !foldedClosureRef) {
  ...
  if (!handoffValid || !priorTurnValid) {
    ...
    if (dispatchError || decision.dispatch.member !== "comms_lead") {
      ...
      proposedTurn.status !== "advanced" ||
      proposedTurn.named_run?.skill !== handoff.skill ||
      proposedTurn.named_run?.data?.content_digest !== handoff.content_digest
    ) {
      result = refused(base, "review act did not preserve the approved digest-bound handoff");
    } else {
      result = {
        incident_turn: {
          ...base,

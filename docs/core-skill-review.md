...
| crm-cleanup | artifact | public/canonical | javascript, tool:data.append_event, tool:data.digest, tool:data.read_events, tool:http.read; 1 agent act -> declared artifact | complete archetype bar | keep | The execution lane fetches one bounded current record, permits only schema-allowlisted updates backed by verbatim transcript evidence, appends one idempotent event, and seals success only after exact provider readback; no-op and refused paths perform no write. | none |

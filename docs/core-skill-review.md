...
Prepared context remains digest-bound and drift-checked for every run, but it is evidence rather than an approval gate. Consequential approval belongs to exactly one action owner: the native effect boundary for provider mutations or an explicit graph gate for a consequential action with no effect-owned gate.
...
checkout evidence rather than a grant, local authenticated reads need no human
approval, and all mutations retain exact approval, idempotency, recovery, and
readback. Hosted Connect continues to own only credential custody, grant
...
compatibility release, and callers that require the old plan-only behavior must
...
| charge | operation | public/canonical | tool:data.compare, tool:payment.charge_challenge, tool:payment.charge_plan, tool:payment.charge_price, tool:payment.charge_verification_request, tool:provider.mutate, tool:provider.read | harness passed; 0 blocking finding(s); 4 operation proof(s); 3 operator journey(s) | improve | The default now binds deterministic price and credential verification to one real configured seller adapter, approval, idempotent settlement, and readback; named rail runners retain plan-only and mock proof. | Capture safe non-mock settlement and recovery evidence before treating paid-operation forwarding as live-proven. |
...
| contract-drafter | artifact | public/canonical | cli-tool:node; 1 agent act -> declared artifact | complete archetype bar | keep | Deterministic package code fetches the bound template, renders only supplied parties and terms, exposes every baseline departure, and refuses missing inputs; one governed send-as planning act consumes the draft-bound proposal while provider delivery remains outside the skill. | none |
...
| github-sync | operation | public/canonical | javascript, tool:data.compare, tool:data.digest, tool:provider.mutate, tool:provider.read | harness passed; 0 blocking finding(s); 5 operation proof(s); 1 operator journey(s); provider readback unproven | improve | The end-to-end default performs the selected bounded pull or push; local authenticated GitHub reads are preferred, writes stay digest-bound and approved, and plan/pull/push remain reusable composed runners. | Capture one safe live pull and one explicitly approved harmless push/readback, including ambiguous-grant refusal evidence. |
...
| messageboard | workflow | public/canonical | tool:data.append_event, tool:data.read_projection; 1 agent act -> declared artifact | complete archetype bar | keep | Direct posting now persists the transition and reads back board state; explicit phase and durable runners preserve the full moderation, claim, delivery, acceptance, and payout-authorization lifecycle. | none |
...
| mock-refund | runtime | internal/harness-fixture | tool:payment.refund_plan | internal; 0 blocking finding(s); not trialled | internal_fixture | Deterministic refund fixture retained for canonical payment integration and recovery tests. | none |
| moltbook | operation | public/context | javascript, tool:data.digest, tool:provider.read; 1 agent act | harness passed; 0 blocking finding(s); 3 operation proof(s); 2 agent-contract proof(s); 3 operator journey(s); provider readback unproven | improve | The default now scans a bounded live provider feed; supplied-snapshot analysis is explicit, and publication remains a separate digest-bound approved mutation/readback lane. | Capture one safe live scan and one explicitly approved harmless post/readback before treating live publication as proven. |
...
| github-pr-comment | workflow | public/canonical | tool:data.compare, tool:data.digest, tool:provider.mutate, tool:provider.read | harness passed; 0 blocking finding(s); 1 operation proof(s); 1 operator journey(s); provider readback unproven | improve | The exact GitHub PR-comment lane binds comment digest, approval, idempotent mutation, narrow result projection, and independent readback across compatible connector or MCP transports. | Capture one safe approved connector comment/readback in addition to the existing MCP harness evidence. |
...
| refund | operation | public/canonical | tool:data.compare, tool:payment.refund_plan, tool:provider.mutate, tool:provider.read | harness passed; 0 blocking finding(s); 2 operation proof(s) | improve | The default links the sealed charge, validates bounded authority, and executes one real configured refund with approval, idempotency, and readback; mock and family planning remain explicit. | Prove bounded non-mock rail execution and recovery against a safe provider sandbox. |
...
| settle-invoice | workflow | public/canonical | external-adapter, tool:data.compare, tool:payment.invoice_plan, tool:payment.quote, tool:payment.reserve; 1 agent act -> declared artifact | complete archetype bar | keep | The default validates the invoice then executes the canonical spend lane on a real configured rail; the explicit plan runner retains the non-mutating handoff and mock proof. | none |
...
| sourcey | workflow | public/context | tool:fs.apply_bundle, tool:sourcey.build, tool:sourcey.package, tool:sourcey.verify; 4 agent acts -> declared artifact | complete archetype bar | keep | Sourcey now proves its bounded authoring, approval, filesystem, deterministic build, critique, revision, rebuild, verification, and packaging stages in one isolated operator journey. | none |
| spend | operation | public/canonical | external-adapter, tool:data.compare, tool:payment.quote, tool:payment.reserve; 1 agent act -> declared artifact | harness passed; 0 blocking finding(s); 2 operation proof(s); 2 agent-contract proof(s); 1 operator journey(s); provider readback unproven | improve | The default selects only a real configured rail, reserves exact authority, gates fulfillment, and requires rail finality; planning and mock execution remain explicit. | Prove non-mock quote, reservation, fulfillment, recovery, and independent provider readback on each supported rail. |
...
| stripe-charge | runtime | internal/runtime-path | tool:payment.charge_challenge, tool:payment.charge_plan, tool:payment.charge_price, tool:payment.charge_verification_request | internal; 0 blocking finding(s); not trialled | internal_runtime | Stripe charge rail retained behind canonical payment skills. | none |
| stripe-pay | operation | public/branded | external-adapter, tool:payment.quote, tool:payment.reserve | harness passed; 0 blocking finding(s); 1 operation proof(s); provider readback unproven | improve | Stripe payment is a valuable branded rail behind canonical spend. | Prove a safe sandbox quote-to-readback path with no agent-authored settlement claim. |
| stripe-refund | runtime | internal/runtime-path | tool:payment.refund_plan | internal; 0 blocking finding(s); not trialled | internal_runtime | Stripe refund rail retained behind canonical payment skills. | none |
...
| x402-pay | operation | public/branded | tool:payment.quote, tool:provider.mutate, tool:provider.read | harness passed; 0 blocking finding(s); 1 operation proof(s) | improve | The default validates the canonical quote then requires a trusted tenant-selected buyer adapter, approval, idempotent payment, and readback; quote-only planning remains explicit. | Prove bounded testnet settlement, recovery, and independent paid-resource readback with a real compatible adapter. |
